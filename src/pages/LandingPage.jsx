...
const programs = [
  ...
  },
  {
    ...
    color: '#1B6B7B', textColor: '#FFFFFF', weeks: 10, days: 5,
  },
  {
    id: 'coming-soon', name: 'More coming', tag: 'ongoing',
    description: 'Half marathon plan, STRNG Part 2, and more added regularly. One subscription, everything included.',
    color: '#F0F0EC', textColor: '#888882', soon: true,
  },
]
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: '#F5F5F2', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <section style={{
        ...
        <div>
          ...
          <h1 style={{
            ...
            lineHeight: 0.88, color: '#0D0D0D', marginBottom: 28,
          }}>
            RUN FAST.<br />
            LIFT<br />
          ...
          <p style={{ fontSize: 17, lineHeight: 1.7, color: '#666660', maxWidth: 460, marginBottom: 44 }}>
      ...
      <section style={{ background: '#FFFFFF', padding: '100px 48px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', gap: 80, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <p style={{ fontSize: 16, lineHeight: 1.8, color: '#555550', maxWidth: 580, marginTop: 16 }}>
              After struggling with RED-S, I had to change the way I train. Now I focus on balance between running, strength, and recovery in a way that supports my health — especially moving through my 40s. These are the exact programs I use to train for races while body-recomping. I'm not a professional runner or influencer — just a real woman with a real body that needs real work. If you're trying to find that same balance between performance, strength, and everyday life — this is for you.
            </p>
          </div>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
          {programs.map(p => (
            <div key={p.id} style={{
              ...
              position: 'relative', overflow: 'hidden', opacity: p.soon ? 0.5 : 1, minHeight: 220,
            }}>
      ...
      <section style={{ background: '#0D0D0D', padding: '100px 48px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
            {features.map((f, i) => (
              <div key={i} style={{ padding: '36px 32px', borderTop: '1px solid #222', borderLeft: i % 3 !== 0 ? '1px solid #222' : 'none' }}>
                <div style={{ fontSize: 32, marginBottom: 16 }}>{f.icon}</div>
                <h3 style={{ fontFamily: 'Bebas Neue', fontSize: 24, color: '#FFFFFF', marginBottom: 10 }}>{f.title}</h3>
                <p style={{ fontSize: 14, lineHeight: 1.65, color: '#666660' }}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer style={{ borderTop: '1px solid #E8E8E4', padding: '32px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, maxWidth: 1200, margin: '0 auto' }}>
        <Wordmark dark />
        <p style={{ fontSize: 12, color: '#888882' }}>© 2026 Sabrina Kyriacou · ZOESTRENGTH</p>
        <div style={{ display: 'flex', gap: 24 }}>
          <a href="#" style={{ fontSize: 12, color: '#888882', textDecoration: 'none' }}>Privacy</a>
          <a href="#" style={{ fontSize: 12, color: '#888882', textDecoration: 'none' }}>Terms</a>
        </div>
      </footer>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 400, margin: centered ? '0 auto' : undefined }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <span style={{ fontFamily: 'Bebas Neue', fontSize: 56, color: '#0D0D0D', lineHeight: 1 }}>
          {billingCycle === 'annual' ? '$99.99' : '$14.99'}
        </span>
        <span style={{ color: '#888882', fontSize: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{
            ...
            border: '1px solid #E0E0DC', fontSize: 13,
            fontFamily: 'DM Sans', background: '#FFFFFF', outline: 'none',
      ...
      <a href={stripeLink} style={{
        ...
        textDecoration: 'none',
        boxShadow: '0 4px 24px rgba(200,245,0,0.3)',
      }}>
        START TRAINING — {billingCycle === 'annual' ? '$99.99' : '$14.99'}
      </a>
      <p style={{ fontSize: 12, color: '#888882', textAlign: 'center' }}>
...
function BoltIcon({ size = 24, color = 'currentColor', style = {} }) {
  return (
    <svg width={size} height={size * 1.3} viewBox="0 0 100 130" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block', ...style }}>
