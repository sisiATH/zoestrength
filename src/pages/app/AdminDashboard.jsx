...
export default function AdminDashboard() {
  ...
  const tabs = ['Programs', 'Build Workout', 'Exercises', 'Extras', 'Analytics']
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F5F2' }}>
      <div style={{ background: '#1B6B7B', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
...
function BuildWorkoutTab() {
  ...
  async function saveExercises(exercisesToSave) {
    ...
    setParsed([]); setPasteText(''); setSelectedFromLibrary([]); setLibraryForms({}); setShowLibrary(false)
    fetchExistingExercises()
    supabase.from('exercises').select('*').order('name').then(({ data }) => data && setExerciseLibrary(data))
    setSaving(false)
  }
        ...
        <div style={{ ...card, marginTop: 16 }}>
          ...
          {!workoutSaved ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <span style={{ fontSize: 14, fontWeight: 700 }}>{workouts.find(w => w.id === selectedWorkout)?.title || 'Workout selected'}</span>
                <span style={{ fontSize: 12, color: '#888882', marginLeft: 8 }}>{existingExercises.length} exercises</span>
              </div>
      ...
      {selectedWorkout && workoutSaved && (
        <div style={{ ...card, marginTop: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
            <button onClick={() => setShowLibrary(false)} style={{ padding: '10px 20px', borderRadius: 100, border: 'none', cursor: 'pointer', background: !showLibrary ? '#0D0D0D' : '#E8E8E4', color: !showLibrary ? '#D4A853' : '#555550', fontFamily: 'DM Sans', fontWeight: 600, fontSize: 13 }}>Paste new exercises</button>
            <button onClick={() => setShowLibrary(true)} style={{ padding: '10px 20px', borderRadius: 100, border: 'none', cursor: 'pointer', background: showLibrary ? '#0D0D0D' : '#E8E8E4', color: showLibrary ? '#D4A853' : '#555550', fontFamily: 'DM Sans', fontWeight: 600, fontSize: 13 }}>Pick from library ({exerciseLibrary.length})</button>
          </div>
          {!showLibrary ? (
            <>
              ...
              {parsed.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  ...
                  <button onClick={() => saveExercises(parsed)} disabled={saving} style={{ ...btn('#1B6B7B'), marginTop: 10 }}>{saving ? 'Saving...' : '✓ Add ' + parsed.length + ' exercises'}</button>
                </div>
              )}
            </>
          ) : (
                  ...
                  return (
                    <div key={ex.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderBottom: '1px solid #F0F0EC', background: selected ? '#F0F8F8' : 'white' }}>
                      <input type="checkbox" checked={selected} onChange={e => {
                        if (e.target.checked) { setSelectedFromLibrary(s => [...s, ex.id]); setLibraryForms(f => ({ ...f, [ex.id]: { sets: 3, reps: '10', rest_seconds: 90, section: '' } })) }
                        else { setSelectedFromLibrary(s => s.filter(id => id !== ex.id)) }
                      }} />
                      <div style={{ flex: 1 }}>
...
function ExercisesTab() {
  ...
  function startEdit(ex) {
    setEditingId(ex.id)
    setEditForm({ name: ex.name, description: ex.description || '', video_url: ex.video_url || '' })
    setMsg('')
  }
  ...
  async function saveEdit() {
    ...
    if (error) setMsg('Error: ' + error.message)
    else { setMsg('Saved!'); setEditingId(null); fetchExercises() }
    setSaving(false)
  }
  ...
  return (
    <div>
      ...
      <div style={card}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search exercises..." style={{ ...inp, marginBottom: 0 }} />
      </div>
            ...
            {editingId === ex.id ? (
              <div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                  <Field label="Name" value={editForm.name} onChange={v => setEditForm(f => ({ ...f, name: v }))} span={2} />
                  <Field label="Description / cues" value={editForm.description} onChange={v => setEditForm(f => ({ ...f, description: v }))} span={2} multiline />
                  <Field label="Video URL (YouTube or embed)" value={editForm.video_url} onChange={v => setEditForm(f => ({ ...f, video_url: v }))} placeholder="https://youtube.com/watch?v=..." span={2} />
                </div>
                {editForm.video_url && (
                  <div style={{ marginBottom: 12 }}>
                    <a href={editForm.video_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: '#1B6B7B', fontWeight: 600 }}>▶ Preview link ↗</a>
                  </div>
                )}
                {msg && <p style={{ color: msg.includes('Error') ? 'red' : 'green', fontSize: 13, marginBottom: 8 }}>{msg}</p>}
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={saveEdit} disabled={saving} style={btn('#1B6B7B', true)}>{saving ? 'Saving...' : 'Save'}</button>
                  <button onClick={() => setEditingId(null)} style={btn('#888882', true)}>Cancel</button>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  {ex.description && <div style={{ fontSize: 11, color: '#888882', marginTop: 3 }}>{ex.description.substring(0, 80)}{ex.description.length > 80 ? '...' : ''}</div>}
                  {ex.video_url
                    ? <div style={{ fontSize: 11, color: '#1B6B7B', marginTop: 4 }}>▶ Video linked</div>
                    : <div style={{ fontSize: 11, color: '#C4857A', marginTop: 4 }}>No video</div>
                  }
                </div>
                <button onClick={() => startEdit(ex)} style={btn('#1B6B7B', true)}>Edit</button>
              </div>
            )}
          </div>
        ))}
...
function AnalyticsTab() {
  ...
  async function fetchAll() {
    setLoading(true)
    await Promise.all([fetchStats(), fetchTopWorkouts(), fetchRecentUsers(), fetchSubBreakdown(), fetchTopExercises()])
    setLoading(false)
  }
  ...
  async function fetchStats() {
    ...
    const { data: activeData } = await supabase.from('workout_completions').select('user_id').gte('completed_at', since)
  ...
  async function fetchTopWorkouts() {
    const { data } = await supabase.from('workout_completions').select('workout_id, workouts(title)')
    if (!data) return
    const counts = {}
    for (const r of data) {
    ...
    setTopWorkouts(Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([title, completions]) => ({ title, completions })))
  }

  async function fetchRecentUsers() {
    const { data } = await supabase.from('profiles').select('id, email, created_at').order('created_at', { ascending: false }).limit(10)
    if (!data) return
    const { data: subs } = await supabase.from('subscriptions').select('user_id, status, plan').in('user_id', data.map(u => u.id))
    const subMap = {}
  ...
  async function fetchSubBreakdown() {
    const { data } = await supabase.from('subscriptions').select('status, plan')
    if (!data) return
    const counts = {}
    for (const s of data) {
  ...
  async function fetchTopExercises() {
    const { data } = await supabase.from('set_logs').select('workout_exercise_id, workout_exercises(exercise_id, exercises(name))').eq('completed', true)
    if (!data) return
    const counts = {}
    for (const r of data) {
    ...
    setTopExercises(Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([name, sets]) => ({ name, sets })))
  }

  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: '#888882' }}>Loading analytics...</div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
        {[
          ...
          { label: 'On Trial', value: stats.trialUsers },
          { label: 'Active (30d)', value: stats.activeUsers },
          { label: 'Completions', value: stats.totalCompletions },
          { label: 'Sets Logged', value: stats.totalSets },
        ].map(s => (
          <div key={s.label} style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E8E4', padding: '16px 20px', flex: 1, minWidth: 120 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#888882', marginBottom: 6 }}>{s.label}</div>
            <div style={{ fontFamily: 'Bebas Neue', fontSize: 32, color: '#0D0D0D', lineHeight: 1 }}>{s.value ?? '—'}</div>
          </div>
        ))}
      </div>
      <div style={card}>
        ...
        {subBreakdown.map(s => (
          <div key={s.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F0F0EC', fontSize: 13 }}>
            <span>{s.label}</span><span style={{ fontWeight: 700 }}>{s.count}</span>
          </div>
        ))}
      </div>
      <div style={card}>
        ...
        {topWorkouts.map((w, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F0F0EC', fontSize: 13 }}>
            <span>{w.title}</span><span style={{ fontWeight: 700 }}>{w.completions}</span>
          </div>
        ))}
      </div>
      <div style={card}>
        ...
        {topExercises.map((e, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F0F0EC', fontSize: 13 }}>
            <span>{e.name}</span><span style={{ fontWeight: 700 }}>{e.sets} sets</span>
          </div>
        ))}
      </div>
      <div style={card}>
        ...
        {recentUsers.map((u, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 70px', gap: 8, padding: '8px 0', borderBottom: '1px solid #F0F0EC', fontSize: 13, alignItems: 'center' }}>
            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</span>
            <span style={{ color: '#888882' }}>{u.joined}</span>
            <span>{u.status}</span>
            <span>{u.plan}</span>
          </div>
        ))}
